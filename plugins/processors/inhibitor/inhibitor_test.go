...
var mockAlerts = map[string]*models.Alert{
	...
	"bgp_2": tu.MockAlert(4, "Neteng BGP Down", "Alert2", "d2", "e2", "src2", "scp2", "4", "INFO", []string{"c", "d"},
		models.Labels{"RemoteDeviceName": "d4"}),
}
...
func TestInhibit(t *testing.T) {
	...
	// test rule no match
	i.addAlert("Device down", mockAlerts["bgp_2"])
	i.checkRule(ctx, rule)
	assert.Equal(t, mockAlerts["bgp_2"].Status, models.Status_ACTIVE)
